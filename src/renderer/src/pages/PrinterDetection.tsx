  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto select-none">
      <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-sm font-black text-slate-900 uppercase tracking-wider">{t.detection.title}</h1>
          <p className="text-xs text-slate-500 font-medium">{t.detection.subtitle}</p>
        </div>
        <button
          onClick={handleScan}
          disabled={isScanning}
        ...
        >
          ...
          {isScanning ? t.detection.scanningPorts : t.detection.scanPorts}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-sm space-y-1.5">
          <div className="flex items-center justify-between text-xs font-extrabold text-blue-700">
            <span className="flex items-center gap-1.5"><Usb className="w-4 h-4 text-blue-600" /> {t.detection.usbPnpMonitor}</span>
            <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded text-[10px] border border-emerald-200">{t.detection.active}</span>
          </div>
          <p className="text-[11px] text-slate-500 font-medium">{t.detection.usbPnpDesc}</p>
        </div>

        <div className="p-3.5 bg-white rounded-xl border border-slate-200 shadow-sm space-y-1.5">
          <div className="flex items-center justify-between text-xs font-extrabold text-emerald-700">
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-emerald-600" /> {t.detection.spoolerSubsystem}</span>
            <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded text-[10px] border border-emerald-200">{t.detection.spoolerReady}</span>
          </div>
          <p className="text-[11px] text-slate-500 font-medium">{t.detection.spoolerDesc}</p>
        </div>
      </div>

      {/* Detected Device Table */}
      <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm space-y-3">
        <h2 className="text-xs font-black uppercase text-slate-900 tracking-wider">{t.detection.enumeratedDevices}</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-slate-100 text-slate-600 uppercase text-[10px] font-extrabold border-b border-slate-200">
              <tr>
                <th className="p-2.5">{t.detection.interface}</th>
                <th className="p-2.5">{t.detection.queueName}</th>
                <th className="p-2.5">{t.detection.driverName}</th>
                <th className="p-2.5">{t.detection.port}</th>
                <th className="p-2.5">{t.detection.status}</th>
              </tr>
            </thead>
